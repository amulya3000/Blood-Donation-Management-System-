...
@WebServlet("/admin/inventory/delete")
public class DeleteBloodServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            inventoryService.removeBlood(request.getParameter("bloodType"), Double.parseDouble(request.getParameter("units")));
            request.getSession().setAttribute("success", request.getParameter("units") + " units deleted from " + request.getParameter("bloodType") + ".");
